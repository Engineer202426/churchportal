...
if (!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== TRUE) {
  header("Location:  storage/ssd1/921/21788921/public_html/thesis/learningsA.php");
  exit;
}
...
function addBook($title, $author) {
  ...
  file_put_contents("./Abook.ini", $updatedIni);
  
  // Create the corresponding .txt file for the content
  $contentFilePath = "./Adult BOOKS/$newContentFile";
  ...
  header("Location: ./learningsA.php?page=$bookNumber");
  exit;
}
...
function removeBook() {
  ...
  header("Location: ./learningsA.php?page=" . max(1, $lastPage));
  exit;
}
...
<body>
  ...
  <div class="container">
    ...
    <div class="book-page">
      ...
      <div id="content">
        <?php
        $contentFilePath = "./Adult BOOKS/$contentFile";
...
function saveEditedContent() {
  ...
  xhr.open("POST", "save_editA.php?page=" + encodeURIComponent(<?php echo json_encode($currentPage); ?>), true);
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        alert("Error saving content: " + xhr.statusText);
        console.error("Error details: ", xhr.responseText);
      }
    }
  };
  xhr.send("editedTitle=" + encodeURIComponent(editedTitle) +
  ...
  if (newTitle && newAuthor) {
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "learningsA.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
